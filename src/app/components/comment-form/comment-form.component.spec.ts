...
describe('CommentFormComponent', () => {
  ...
  describe('onInit()', () => {
    it('should set form controls if content is provided and isUpdateComment true', () => {
      ...
      component.ngOnInit();
      expect(component.commentForm.controls['user'].value).toEqual(component.formContent.user);
      expect(component.commentForm.controls['comment'].value).toEqual(component.formContent.content);
    });
  })
  ...
  describe('pushComment()', () => {
    it('should call emit on updateComment with create comment object',
    inject([DatePipe], (datePipe: DatePipe) => {
      ...
      expect(component.updateComment.emit).toHaveBeenCalledWith(expectedResult);
    }));

    it('should call emit on updateComment with create reply object',
    inject([DatePipe], (datePipe: DatePipe) => {
      const expectedResult = {
        comment: {
          content: 'Mock Comment',
    ...
    it('should call emit on updateComment with update comment object',
    inject([DatePipe], (datePipe: DatePipe) => {
      const expectedResult = {
        comment: {
