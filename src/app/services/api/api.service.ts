...
export class ApiService {
  ...
  public getPosts(): Observable<BlogPostModel[]> {
    return this.http.get<BlogPostModel[]>(`${this.baseUrl}/posts?_sort=publish_date&_order=desc`);
  }
  ...
  public searchPosts(searchTerm: string): Observable<BlogPostModel[]> {
    return this.http.get<BlogPostModel[]>(`${this.baseUrl}/posts?q=${searchTerm}&_sort=publish_date&_order=desc`);
  }

  public getPostComments(postId: number): Observable<CommentModel[]> {
    return this.http.get<CommentModel[]>(`${this.baseUrl}/posts/${postId}/comments?_sort=date&_order=desc`);
  }

  public createPostComment(postId: number, comment: CommentModel): Observable<any> {
