...
describe("CommentService", () => {
  ...
  beforeEach(() =>
    TestBed.configureTestingModule({
      providers: [HttpClientTestingModule, HttpHandler, HttpClient]
    })
  );
  ...
  describe("createComment()", () => {
    it("should call createPostComment and loadComments", inject(
      [ApiService],
      (apiService: ApiService) => {
        ...
        expect(apiService.createPostComment).toHaveBeenCalledWith(
          comment[0].postId,
          comment[0]
        );
  ...
  describe("updateComments()", () => {
    it("should call updatePostComment", inject(
      [ApiService],
      (apiService: ApiService) => {
        let service = new CommentService(apiService);
  ...
  describe("deleteComment()", () => {
    it("should call deleteComment", inject(
      [ApiService],
      (apiService: ApiService) => {
        let service = new CommentService(apiService);
        ...
        expect(apiService.deletePostComment).toHaveBeenCalledWith(
          comment[0].id
        );
        expect(service.loadComments).toHaveBeenCalled();
      }
